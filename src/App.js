...
class App extends Component {
  ...
  render() {
    const {activeTabId, timerCount, count, projectUrl, isTrue} = this.state
    console.log(isTrue)
    const imagelistData = imagesList.filter(
      eachData => eachData.category === activeTabId,
    )
    ...
    if (timerCount < 1) {
      ...
      sai = (
        <div className="afternavbarcontainer">
          <img src={projectUrl} alt="match" className="randomimageone" />
          <ul className="unorderlisttabitems">
            {tabsList.map(each => (
              <TabItems
                key={each.tabId}
                each={each}
                onTabItemIdOne={this.onTabItemIdOne}
                isActive={each.tabId === activeTabId}
              />
            ))}
          </ul>
          <ul className="secondunorderlist">
            {imagelistData.map(eachOne => (
              <ProjectList
                key={eachOne.id}
                eachOne={eachOne}
                onImageprojectList={this.onImageprojectList}
              />
            ))}
          </ul>
        </div>
          ...
          <li className="anothernavcontainer">
            ...
            <li className="thirdnavcontainer">
              <img
                ...
                alt="timer"
                className="websitelogoimageone1"
              />
              <p className="scoreparagraph">{timerCount} sec</p>
            </li>
          </li>
        </ul>
        {sai}
